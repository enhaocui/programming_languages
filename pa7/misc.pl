...
% availabe_at returns true when X is available at Y, the function goes 
%through each taqueria and check of X in different Zs
available_at(X,Y) :- taqueria(Y, _, Z), isin(X, Z).

% mult_available returns true if x is available in multiple items
% use bagof to go through all that return true for available at
% and then check for the results with more than 1 elements
multi_available(X) :- bagof(Y, available_at(X, Y), P), P = [_, _|_]. 

% overworked returns true if person X works at more than one taq.
% works_at serves as helper function similar to what available_at does: if one person
% is available at Y
works_at(X, Y):- taqueria(Y, Z, _), isin(X,Z).

overworked(X) :- bagof(Y, works_at(X, Y), P), P = [_, _|_]. 

% total_cost returns true if the sum of the item X is K
% sum_cost serves as a helper function to calculate the cost of a certain element, 
%and then call itself recursively on the remining of the elements
sum_cost([], K) :- K is 0.

sum_cost([H|T], K) :- cost(H, Y), sum_cost(T, R), K is Y + R. 

total_cost(X,K) :- ingredients(X, Y), sum_cost(Y, K). 

% has ingredients returns true if X has all the ingredients in L
% We can check for all the ingredients Y of X, and then check if the intersection of Y and L is L
has_ingredients(X,L) :- ingredients(X, Y), intersection(Y, L, Y).

% avoids ingredients returns true if X has none of the ingredients in L
% We can check for all the ingredients Y of X, and then check if the intersection of Y and L is empty
avoids_ingredients(X,L) :- ingredients(X, Y), intersection(Y, L, []).

%p1 returns items that has all the ingredients in X
p1(L,X) :- bagof(Y, has_ingredients(Y, X), L). 

%p1 returns items that has none of the ingredients in Y
