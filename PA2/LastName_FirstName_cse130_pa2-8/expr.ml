...
let rec exprToString e = 
  ...
  | Absin(t) -> "abs(sin(" ^ exprToString t ^ "))"
  | Max(t, p, m) -> "max(" ^ exprToString t ^ "," ^ exprToString p ^ "," ^ exprToString m ^ ")";;
...
let buildAbsin(a)                  = Absin(a)
let buildMax(t,p,m)                = Max(t,p,m)
...
let rec eval (e,x,y) = 
  ...
  | Absin(t) -> abs_float(sin(eval(t, x, y)))
  | Max(t,p,m) -> max (eval(t, x, y)) (max (eval(p, x, y)) (eval(m, x, y)));;
