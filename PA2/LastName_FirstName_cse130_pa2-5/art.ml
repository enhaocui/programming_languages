...
let rec build (rand,depth) = 
  let x = rand(0,8) in 
    ...
    | 0 -> buildSine(build(rand, depth - 3))
    | 3 -> buildCosine(build(rand, depth - 1))
    | 4 -> buildAverage(build(rand, depth - 3),build(rand, depth - 1))
    | 1 -> buildAverage(build(rand, depth - 1),build(rand, depth - 2))
    | 5 -> buildTimes(build(rand, depth - 1),build(rand, depth - 2))
    | 6 -> buildThresh(build(rand, depth - 3),build(rand, depth - 1),build(rand, depth - 2),build(rand, depth - 1))
    | _ -> buildY();;
